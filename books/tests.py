...
class BookModelTests(TestCase):
    ...
    def test_update_average_rating(self):
        ...
        Review.objects.create(book=self.book, reviewer=user, rating=4, comment='Good book!')
        new_user = User.objects.create_user(username='another_testuser', password='testpassword')
        Review.objects.create(book=self.book, reviewer=new_user, rating=5, comment='Excellent read!')
...
class BookSerializerTests(TestCase):
    ...
    def test_serializer_fields(self):
        ...
        self.assertEqual(data['average_rating'],
                         format(float(self.book.average_rating), '.2f'))  # Compare to formatted average_rating
